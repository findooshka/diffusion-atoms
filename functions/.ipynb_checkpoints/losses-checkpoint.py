...
def lattice_loss(model,
    ...
    #lattice_noise_scaled = lattice_noise * s_t.sqrt()
    ...
    noise_estimate = get_output(noised_atoms, model, t, device, output_type="lattice", b6=noised_transformed_lattice6, gradient=gradient, emax=50)[1]
...
def noise_estimation_loss(model,
    ...
    if False:
        ...
        pos_eps, lattice_eps = get_output(noised_atoms, model, t, device, output_type="atoms")
        bce = torch.nn.BCEWithLogitsLoss()
        loss = bce(fake_estimate, torch.tensor(fake, device=device).view(-1, 1))
        loss *= count_loss_coef
    else:
        pos_eps, lattice_eps = get_output(noised_atoms, model, t, device, output_type="edges")
