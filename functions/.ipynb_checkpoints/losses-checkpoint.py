...
def elements_loss(model,
    ...
    element_estimate = get_output(noised_atoms, operations, space_group, sg_type, model, t, device, output_type="atoms")[2]
    true_numbers = np.array(atoms.atomic_numbers) - 1
    truth = torch.zeros((len(atoms.elements), 95), device=device, dtype=element_estimate.dtype)
    truth[np.arange(0, truth.shape[0]), true_numbers] = 1
    loss = (element_estimate - truth).square()
    #result = loss.clone()
    type2_error_coef = torch.tensor(element_loss_coef[true_numbers], device=device, dtype=element_estimate.dtype)
    #result[np.arange(0, truth.shape[0]), true_numbers] = type2_error_coef * loss[np.arange(0, truth.shape[0]), true_numbers]
    loss[np.arange(0, truth.shape[0]), true_numbers] *= type2_error_coef
